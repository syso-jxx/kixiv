...
<body>
	...
	<h1 class="text-dark h1" style="text-align:left;">투자 리스트</h1>
	<br/>
						...
						<c:when test="${list.size() > 0}">
							<c:forEach var="invest" items="${list}" varStatus="status">
								<tr style="cursor:pointer;" onclick="javascript:location.href='fundDetail?fund_id=${invest.FUND_ID}'">
									...
									<td>${invest.RATE}%</td>
									<td>${invest.ONE_PRICE}원</td>
								</tr>
							</c:forEach>
						</c:when>
						<c:otherwise>
							<tr>
								<td colspan="7" rowspan="10">데이터가 없습니다.</td>
							</tr>
						</c:otherwise>
					</c:choose>
				</tbody>				
		...
		<div class="col-md-offset-3">
			<ul class="pagination justify-content-center">
				<c:if test="${page.prev}">
					<li class="page-item">
						<a class="page-link" href="/investList?num=${page.startPageNum - 1}">이전</a>
					</li>
				</c:if>
				...
				<c:forEach begin="${page.startPageNum}" end="${page.endPageNum}" var="num">
					<c:choose>
						<c:when test="${page.num != num}">
							<li class="page-item"><a class="page-link" href="/investList?num=${num}">${num}</a></li>
						</c:when>	
						<c:otherwise>
				...
				<c:if test="${page.next}">
					<li class="page-item">
						<a class="page-link" href="/investList?num=${page.endPageNum + 1}">다음</a>
					</li>
				</c:if>
			</ul>
		</div>
